...
public class TrailerVideoPlayerActivity extends Activity {
	...
	private void startPlayer(int index) {
		...
		videoUri = Uri.parse(AppConstants.trailervideosList.get(index).getUrl());
	...
	private final MediaPlayer.OnCompletionListener videoViewOnCompletionListener = new MediaPlayer.OnCompletionListener() {
		public void onCompletion(MediaPlayer mp) {
			...
			currVideoIndex += 1;
			if (currVideoIndex < AppConstants.trailervideosList.size()) {
				imageViewLoader.DisplayImage(
						AppConstants.trailervideosList.get(currVideoIndex)
